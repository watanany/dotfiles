...
flake.nix                      ← 玄関。inputs(外部リポ) と outputs(構成) を宣言
 │
 ├── hosts/nixos/              ← このマシン固有 (hostname, hardware)
 │     ├── default.nix
...
読む順序のおすすめ: `flake.nix` → `hosts/nixos/default.nix` → `modules/*.nix` → `home/default.nix`
...
| 用途 | コマンド |
|---|---|
| 設定を適用 | `sudo nixos-rebuild switch --flake .#nixos` |
| 適用せずビルドだけ | `nixos-rebuild build --flake .#nixos` |
